...
#include <stdio.h>
#include <stdlib.h>
#define N 800// Разбиение заданного участка определяется константой
...
int main()
{
	...
	start=0; finish=4; step=0.005; // Присваивание переменным соответствующих значений
	solutionN[0]=0;// Решение "диффура" в начальной точке по условию задачи

	for (int k=0; k<N; k++)// Цикл реализует решение "диффуров" методом Рунге-Кутта(ы) четвертого порядка 
	{
		...
		printf(" %.7f \n", solutionN[k]); // Сравнение результатов вычисления решений "диффура" численно и аналитически с указанием шага. Количество знаков после запятой увеличено для наблюдения погрешности, т.к. метод Рунге-Кутта считается довольно точным, погрешности начинают появлятся после 4-го, 5-го знака после запятой
	};
...
{
	return 0.5+0.5*x+cos(x)+sin(x)-y;
}
